...
const Cart = () => {
  const items = useCartStore(
    (state: { items: CartItem[] }) => state.items as CartItem[]
  );
  const updateQuantity = useCartStore(
    (state: { updateQuantity: (book: Book, quantity: number) => void }) =>
      state.updateQuantity
  );
  const removeFromCart = useCartStore(
    (state: { removeFromCart: (book: Book) => void }) => state.removeFromCart
  );
